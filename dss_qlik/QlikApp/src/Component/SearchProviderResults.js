...
export default class SearchProviderResults extends Component {
  ...
  setExtract(extracts) {
    ...
    try {
      const extractDate = extracts[0]['ExtractDate'] + 'T' + extracts[0]['ExtractTime']
      const parsedDate = parseISO(extractDate)
      const formatDate = format(parsedDate, 'd MMMM yyyy h:mma')

      this.setState({
        extract_date: formatDate
      })

      this.props.announcer.appendMessages([
        'Provider Register data was extracted on ' + formatDate
      ])
    } catch (error) {
      console.log(error)
    }
  ...
  render() {
    let resultsFound = this.state.total + ' of ' + this.hypercube.total +
    ' results found. Results are displayed in alphabetical order.'
