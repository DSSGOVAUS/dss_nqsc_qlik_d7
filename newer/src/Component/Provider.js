...
export default class Provider extends Component {
  ...
  setRegistrationGroups(results) {
    if (!results[0].RegistrationGroup_Name) {
      return
    }
    ...
    this.props.announcer.appendMessages([
      'Registration groups',
      registrationGroupsFormat(results)
    ])
  }
  ...
  setRegistrationConditions(results) {
    if (!results[0]['RegistrationConditions/Name']) {
      return
    }
    ...
    this.props.announcer.appendMessages([
      ...
      registrationConditionsFormat(results)
    ])
  }
          ...
          <div className="field-wrapper">
            ...
            <span className="field-value">
              {this.state.provider.ScheduleActivityEndDate && (
                format(
                  new Date(this.state.provider.ScheduleActivityEndDate),
                  'Do MMMM, YYYY'
                )
              )}
            </span>
